...
// insert Coinscore
app.post("/coinscore", async (request, response) => {

  // receive and log the coinscore data from the
  let coinscore = request.body;
	console.log("Inserting coinscore: " + JSON.stringify(coinscore));

  // connect to the database and insert the coinscore data
  let db = await MongoClient.connect(url);
  let result = await db.db("mydb")
  .collection("coinscores").insertOne(coinscore);

  // log the the result and send response to client
  console.log("Coiscoren: " + JSON.stringify(coinscore) + " blev indsat")
  response.json(coinscore);

  // close the database
  db.close();
});

// Insert the Name and the Age
app.post("/namenage", async (request, response) => {

  // recieve and log the name and age
  let namenage = request.body;
  console.log("Inserting the name and age: " + JSON.stringify(namenage));

  // connect to the database and inserts the data
  let db = await db.db("mydb")
  .collection("namenage").insertOne(namenage);

  // log the result and send the response to the terminal
  console.log("The name and age was inserted: " + JSON.stringify(namenage))
  response.json(namenage);
